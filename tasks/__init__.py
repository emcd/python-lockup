...
def build_python_venv( context, version, overwrite = False ):
    ...
    fixtures = calculate_python_packages_fixtures( context, context_options )
...
def clean_python_packages( context, version = None ):
    ...
    installed = frozenset(
        ...
        for entry in indicate_current_python_packages(
...
def calculate_python_packages_fixtures( context, context_options ):
    fixtures = [ ]
    for entry in indicate_current_python_packages( context, context_options ):
...
def indicate_current_python_packages( context, context_options ):
...
def retrieve_pypi_release_information( name, version, index_url = '' ):
